...
- **Deferred toolchain upgrade (Kotlin / KSP / Hilt / AGP / Gradle).** Pinned together because they're coupled and can't advance piecemeal on the current stack. Held at Kotlin 2.0.21, KSP 2.0.21-1.0.28, Hilt 2.52, AGP 8.13.2, Gradle 8. The blockers, traced from CI:
  ...
  - A verified intermediate combo exists that stays on AGP 8 / Gradle 8 (Kotlin 2.2.21 + KSP 2.2.21-2.0.5 + `ksp.useKSP2=false` + Hilt 2.58 — full CI green), but it leans on the soon-to-be-removed KSP1 backend, so it's a stepping stone rather than a destination. The real target is the coordinated major bump: AGP 9 + Gradle 9 + Kotlin 2.4+ (KSP2 once its bug is fixed) + Hilt 2.59+. `renovate.json` holds the Kotlin/KSP/Dagger group behind dashboard approval and caps AGP/Gradle below v9 so the broken individual PRs stop; lifting those is the trigger to do the upgrade deliberately.
