...
- **Deferred toolchain upgrade (Kotlin / KSP / Hilt / AGP / Gradle).** Pinned together because they're coupled and can't advance piecemeal. Currently on the intermediate stepping stone (see below): Kotlin 2.2.21, KSP 2.2.21-2.0.5 (KSP1 backend via `ksp.useKSP2=false`), Hilt 2.58, still on AGP 8.13.2 / Gradle 8. The remaining blockers to the AGP 9 / Gradle 9 destination, traced from CI:
  - KSP changed its versioning: `…-x.y.z` (paired to Kotlin, ships the stable **KSP1** backend) through `2.2.21-2.0.5`; then decoupled semver `2.3.x`, which is **KSP2-only** and needs Kotlin 2.3+. The KSP2 (Analysis-API) backend currently crashes this project with `KspAAWorkerAction: unexpected jvm signature V`.
  - Hilt 2.52's bundled `kotlinx-metadata-jvm` only reads Kotlin metadata ≤ 2.1.0, so any Kotlin 2.2+ bump needs Hilt ≥ ~2.56.
  - Hilt 2.59+ requires **AGP 9.0+**, which requires **Gradle 9** (and likely a CI JDK bump).
  - **Applied (2026-06-26):** this intermediate combo, which stays on AGP 8 / Gradle 8 — Kotlin 2.2.21 + KSP 2.2.21-2.0.5 + `ksp.useKSP2=false` + Hilt 2.58 (CI green: unit tests pass). It leans on the soon-to-be-removed KSP1 backend, so it's a stepping stone rather than a destination. The bump raised the compiler metadata floor to 2.2.0, which let `kotlinx-coroutines` 1.11.0 and `mockk` 1.14.11 ride along (the two renovate PRs it unblocked). The real target remains the coordinated major bump: AGP 9 + Gradle 9 + Kotlin 2.4+ (KSP2 once its bug is fixed) + Hilt 2.59+. `renovate.json` still holds the Kotlin/KSP/Dagger group behind dashboard approval and caps AGP/Gradle below v9 (the manual bump here was done deliberately, not via those PRs); lifting those caps is the trigger to do the AGP 9 upgrade.
  - **Some AndroidX libraries now also need AGP 9 and join this wall**, even though they're minor bumps in the otherwise-flowing AndroidX group: `androidx.core` ≥ 1.19.0 and `androidx.lifecycle` ≥ 2.11.0 require AGP 9.1.0 + compileSdk 37 (the AAR-metadata check fails the build otherwise). `renovate.json` caps `androidx.core:**` below 1.19.0 and `androidx.lifecycle:**` below 2.11.0 so the rest of AndroidX keeps updating; lift those caps as part of the coordinated AGP 9 / Gradle 9 upgrade. Expect more AndroidX libs to cross this line over time.
  - **Room is held at 2.6.x** (`renovate.json` caps `androidx.room:**` below 2.7.0) for a *different* reason than the AGP 9 wall. Room 2.7.0 replaced Gson with kotlinx-serialization for schema-bundle parsing; because `exportSchema = true`, the room-compiler deserializes the schema JSON during KSP, and Room 2.7+'s generated `$$serializer` classes are built against a newer kotlinx-serialization-core than the pinned `kotlinxSerialization = 1.7.3` — KSP fails with `AbstractMethodError: ...typeParametersSerializers()`. Fixing it means bumping kotlinx-serialization — and CI confirms the current target `kotlinx-serialization` 1.11.0 (renovate #16) ships metadata 2.3.0, i.e. it requires Kotlin 2.3+, which forces the broken KSP2 backend. So serialization stays at 1.7.3 on the current 2.2.21 stepping stone and folds into the coordinated AGP 9 / Kotlin 2.4+ upgrade alongside Room. To lift: bump `kotlinxSerialization` in lockstep with Room and re-verify KSP schema export.
